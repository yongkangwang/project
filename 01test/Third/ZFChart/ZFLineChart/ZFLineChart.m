...
- (void)commonInit{
    ...
    _overMaxValueCircleNum = CGFLOAT_MAX;
    _overMinValueCircleNum = CGFLOAT_MIN;
...
- (void)drawCircle{
    ...
    if ([subObject isKindOfClass:[NSString class]]) {
        for (NSInteger i = 0; i < self.genericAxis.xLineValueArray.count; i++) {
            ...
            float yNum = [self.genericAxis.xLineValueArray[i] floatValue];
            
            if (yNum>=self.overMaxValueCircleNum || yNum<=self.overMinValueCircleNum) {
                isOverrun = YES;
            }
        ...
        if ([[subObject firstObject] isKindOfClass:[NSString class]]) {
            ...
            for (NSInteger lineIndex = 0; lineIndex < self.genericAxis.xLineValueArray.count; lineIndex++) {
                ...
                for (NSInteger circleIndex = 0; circleIndex < [subObject count]; circleIndex++) {
                    ...
                    float yNum = [self.genericAxis.xLineValueArray[lineIndex][circleIndex] floatValue];
